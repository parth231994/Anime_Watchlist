...
import './login.css';
// import Link from './RegisterUserComponent';
...
handleSubmit = async (event) => {
    ...
    if(isEmpty(username)){
        ...
        errors.usernameMessage = "This fieled is required";
    }
    
    if(!isEmpty(username) && (username.length < 3)){
        errors.username = true;
        errors.usernameMessage = "This fieled requires min 3 characters";
    }


    if(isEmpty(password)){
        errors.password = true;
        errors.passwordMessage = "This fieled is required";
    }
      ...
      <div className="login-form-1">
        <form id="register-form" className="text-left" onSubmit={this.handleSubmit}>
          ...
          <div className="main-login-form">
            ...
            <button type="button" className="btn btn-secondary" style={{marginLeft: '55%'}} onClick={() =>this.handleCancel()}>Cancel</button>           
            {/* <Link to="/RegisterUserComponent" className="btn btn-link">Register</Link> */}
